...
var Store = function () {
    ...
    _createClass(Store, [{
        ...
        value: function setState(newState, cb) {
            ...
            // get listeners
            var listeners = Object.values(this._vlowListeners_);
        ...
        value: function _vlowSetState(listener, state, counter) {
            ...
            if (Object.keys(state).length) {
                ...
                switch (component._vlowState_) {
                    case _states2.default.init:
                        counter ? process.env.NODE_ENV !== 'production' ? (0, _invariant2.default)(false, 'Store `%s` is using `setState()` with a callback on component `%s` which is not yet mounted. This is not possible, make sure all components are mounted or remove the callback from setState.', this.constructor.name, component.constructor.name) : (0, _invariant2.default)(false) : undefined;
                        component.state = Object.assign(component.state || {}, state);
                        break;
                    case _states2.default.ready:
                        component.setState(state, counter);
                        break;
                    case _states2.default.stop:
                        counter && counter.i--;
                        break;
                }
            } else {
        ...
        value: function _vlowAddListener(component, keys, altState) {
            ...
            var listener = { component: component, keys: keys, altState: altState };
        ...
        value: function _vlowGetOrCreateStore(StoreClass) {
            ...
            _stores.push({ class: StoreClass, store: store });
            return store;
        }
    }]);
